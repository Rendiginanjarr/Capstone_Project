...
def plot_age(data):
    ...
    def age_grouping(data):
        if(data.age <= 24):
            ...
            return '24 - 30'
        elif(data.age > 31 and data.age <= 35) : 
            return '31 - 35'
        elif(data.age > 36 and data.age <= 40) : 
            return '36 - 40'
        elif(data.age > 41 and data.age <= 45) : 
            return '41 - 45'
        elif(data.age > 46 and data.age <= 50) : 
            return '46 - 50'
        elif(data.age > 51 and data.age <= 55) : 
            return '51 - 55'
        elif(data.age > 56 and data.age <= 59) : 
            return '56 - 59'
        else : 
            return '60+'
...
def plot_state(data):
    ...
    df_state = data[data.fraud_reported == 'Y'].pivot_table(index='incident_city',values='fraud_reported',aggfunc='count')
    df_nstate = data[data.fraud_reported == 'N'].pivot_table(index='incident_city',values='fraud_reported',aggfunc='count')


    p_state_report = pd.concat([df_state,df_nstate],axis=1)


    ax = p_state_report.plot.bar(stacked = True,color =['#c34455','#5fba7d'],title = "Fraud by Incident City Report ", figsize=(8, 6))
...
def plot_hour(data):
    ...
    df_incident_hour = data[data.fraud_reported == 'Y'].pivot_table(index='incident_hour_of_the_day',values='fraud_reported',aggfunc='count')

    p_incident_report = pd.concat([df_incident_hour],axis=1)

    ax = p_incident_report.plot(legend=True, title = "Number of Fraud per Incident Hour of The Day",color='#5fba7d', figsize=(8, 6))
